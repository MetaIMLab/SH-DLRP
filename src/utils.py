...
def give_gaussian(img, p=0.1):
    ...
    if do_it:
        ...
        blur_layer = get_gaussian_kernel(channels=img.shape[1]).to(img.device)
...
def give_data_change(image):
    image1 = give_gaussian(image.clone(), p=0.5)
    image2 = give_gaussian(image.clone(), p=0.1)
...
def cosine_scheduler(base_value, final_value, epochs, niter_per_ep, warmup_epochs=0, start_warmup_value=0):
    ...
    schedule_length = max(1, len(iters) - 1)
    schedule = final_value + 0.5 * (base_value - final_value) * (1 + np.cos(np.pi * iters / schedule_length))
...
def common_params(student_model: nn.Module, teacher_model: nn.Module, accelerator: Accelerator):
    params_q = dict(accelerator.unwrap_model(student_model).named_parameters())
    params_k = dict(accelerator.unwrap_model(teacher_model).named_parameters())
    names_common = [name for name in params_q if name in params_k]
    return [params_q[name] for name in names_common], [params_k[name] for name in names_common]
...
def give_masks(images, patch_size, pred_ratio, pred_ratio_var):
    ...
    for images_data in images:
        ...
        high = int(get_pred_ratio(pred_ratio, pred_ratio_var) * H * W * Z)
        high = min(max(high, 0), H * W * Z)
        log_aspect_ratio = tuple(map(lambda x: math.log(x), (0.3, 1 / 0.3)))
        mask = np.zeros((H, W, Z), dtype=bool)
        mask_count = 0
        # 总体值开3次方
        mask_num = max(1, int(pow(high, float(1) / float(3)))) if high else 0
        attempts = 0
        while mask_count < high and attempts < 1000:
            attempts += 1
            h = min(mask_num, H)
            w = min(mask_num, W)
            z = min(mask_num, Z)
            ...
            region = mask[top: top + h, left: left + w, hig: hig + z]
            available = np.flatnonzero(~region)
            if available.size == 0:
                continue
            take = min(high - mask_count, available.size)
            region.flat[available[:take]] = True
            mask_count += take

        if mask_count < high:
            remaining = np.flatnonzero(~mask)
            mask.flat[remaining[:high - mask_count]] = True
    ...
    if not masks:
        raise ValueError("No valid image tensors were provided for mask generation")
    return torch.stack(masks, dim=0).to(images.device)


class Logger(object):
    def __init__(self, logdir: str):
        self.console = sys.stdout
        self.error_console = sys.stderr
        self.closed = False
