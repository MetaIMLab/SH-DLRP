...
class MultiModalVisionTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        has_logits =False
        for i in range(3):
            self.vits.append(vit_base_patch16_224_in21k(num_classes=128, has_logits=has_logits))

        if self.use_pretrain:
            assert os.path.exists(self.model_dir), "weights file: '{}' not exist.".format(self.model_dir)
            weights_dict = torch.load(self.model_dir, map_location='cuda' if torch.cuda.is_available() else 'cpu')
            # 删除不需要的权重
    ...
    def forward(self, img_swt, img_swv, img_bus, report, swt_fea, swv_fea, bus_fea):
        ...
        out_1 = self.vits[0](img_swt)
        out_2 = self.vits[1](img_swv)
        out_3 = self.vits[2](img_bus)
        ...
        # weight = self.softmax(torch.cat((out_score,  fea_2_score), dim=-1)).unsqueeze(dim=-1).repeat(1,1,128)
        ...
        out_value = weight_value.view((b_size, 128 *3))
...
if __name__ == '__main__':
    ...
    model = MultiModalVisionTransformer(True, "../../pretrain_weights/jx_vit_base_patch16_224_in21k-e5005f0a.pth").to(device)
