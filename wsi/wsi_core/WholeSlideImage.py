...
import h5py
import math
from wsi.wsi_core.wsi_utils import savePatchIter_bag_hdf5, initialize_hdf5_bag, coord_generator, save_hdf5, sample_indices, \
    screen_coords, isBlackPatch, isWhitePatch, to_percentiles
import itertools
from wsi.wsi_core.util_classes import isInContourV1, isInContourV2, isInContourV3_Easy, isInContourV3_Hard, \
    Contour_Checking_fn
    ...
    def block_blending(self, img, vis_level, top_left, bot_right, alpha=0.5, blank_canvas=False, block_size=1024):
        ...
        for x_start in range(top_left[0], bot_right[0], block_size_x * int(downsample[0])):
            for y_start in range(top_left[1], bot_right[1], block_size_y * int(downsample[1])):
                ...
                # 1. convert wsi_raw coordinates to image coordinates via shift and scale
                ...
                if not blank_canvas:
                    # 4. read actual wsi_raw block as canvas block
                    pt = (x_start, y_start)
